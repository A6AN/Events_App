...
const LIVE_EVENTS = [
    ...
    {
        id: '3',
        title: 'Rooftop Sundowner x DJ Rave',
        category: 'MUSIC',
        image: 'https://images.unsplash.com/photo-1470225620780-dba8ba36b745?auto=format&fit=crop&q=80&w=800',
        location: 'Skybar, Worli',
        time: 'Tomorrow, 5 PM',
        price: 'Free',
        attendees: 89,
        isLive: false,
                ...
                {tab === 'live' ? (
                    <>
                        ...
                        {LIVE_EVENTS.map((event, i) => (
                            <div key={event.id} className="poster-card" onClick={() => navigate(`/event/${event.id}`)} style={{ animationDelay: `${i * 0.1}s` }}>
                                ...
                                <div className="poster-price">{event.price}</div>

                                <div className="poster-info">
                                    <div className="poster-category">{event.category}</div>
                                    <h3 className="poster-title">{event.title}</h3>
                                    <div className="poster-meta">
                                        <span className="poster-meta-item"><MapPin size={13} /> {event.location}</span>
                                        <span className="poster-meta-item"><Clock size={13} /> {event.time}</span>
                                    </div>
                                    <div className="poster-bottom">
                                        <div className="poster-avatars">
                                            ...
                                            <span className="poster-going">+{event.attendees} going</span>
                                        </div>
                                        <div className="poster-fire">🔥 {event.fireCount}</div>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <>
                        ...
                        <div className="masonry-grid">
                            {FRIEND_EVENTS.map(event => (
                                <div key={event.id} className="masonry-card" onClick={() => navigate(`/event/${event.id}`)}>
